...
const lowUILine1 string = "^?: COMMENT LINE | ^C: STORE LINE         | ^R: REPLACE ALL     | ^S: SAVE"
const lowUILine2 string = "^X: DELETE LINE  | ^V: INSERT STORED LINE | ^T: TOGGLE LINE NO. | ^Q: QUIT"

const ReplaceUI string = "REPLACE ALL: 'a'/'b' [malformed input will be ignored]: "

// struct for handling text editing (insert, delete etc)
type TextEditor struct { //fields are organised by visibility -> type
	path          string
	tab           string
	comment       string
	storedLine    string
...
// removes line
func (t *TextEditor) XLine() {
	if len(t.text) <= 1 { //if there is 1 line left in the file
		t.text[t.cur.Y()] = ""
		t.cur.ScrollToLineStart()
	} else if t.cur.Y() >= len(t.text)-1 {
		t.cur.ScrollVertical(t.text, t.termHeight, -1)
		t.cur.ScrollToLineEnd(t.text)
...
// stores current line (without whitespace)
func (t *TextEditor) StoreLine() {
	line := t.text[t.cur.Y()]
	line = strings.TrimSpace(line)
	t.storedLine = line
}

func (t *TextEditor) InsertStoredLine() {
	if t.storedLine != "" { //only pasting the stored line if is not empty (to stop saved from being incorrectly set to true)
		t.InsertString(t.storedLine)
	}
}
