...
const helpMessage string = "\n\r" +
    ...
    "\n\r" +
    "flags:\n\r" +
    "> NCT can be run with the flags '-help', '-settings' & '-n'\n\r" +
    "> running with the '-help' flag instead of a filepath will display a help message\n\r" +
    "> running with the '-settings' flag instead of a filepath will open settings (with no syntax highlighting)\n\r" +
    "> running with the '-n' flag after the filepath will open the file with no syntax highlighting\n\r" +
    "\n\r" +
    "settings:\n\r" +
    ...
    "> only one comment & multi-line comment can be configured per file extension - the latest entry takes priority\n\r" +
    "> valid types are: 'keyword', 'symbols', 'strings', 'comment' & 'mlincom' (for multi-line code comments)\n\r" +
    ...
    "> note - identifiers for symbol entries must be single-char symbols (whitespace, underscores & common string symbols are invalid)\n\r" +
    "> note - identifiers for string entries must be one of: [\", ', `]\n\r" +
    "> note - identifiers for multi-line code comments must include a closing & opening identifier, split by a single space\n\r" +
    ...
    "> tabs are 4 spaces\n\r" +
    "> only the first instance of a multi-line code comment identifier on any given line will be recognised - subsequent identifiers will be highlighted like regular lines\n\r" +
    "> any function that takes in a range of values low/high is inclusive, where low & high are clamped to their respective file ends\n\r" +
...
func getReplaceAllInp(oldState *term.State, inpMsg string) (string, string, bool, error) {
    ...
    if a == trimA || b == trimB || len(trimA) == 0 { //checking for surrounding ' & if a is empty
        return "", "", false, nil
    }
    return trimA, trimB, true, nil
}

//parses non-raw user input into a & b for TextEditor methods that take in a range, returns true if input is valid
...
func editMode(editor *texteditor.TextEditor, hueMap *huesettings.HueMap) error {
    ...
    for editLoop := true; editLoop; {
        ...
        case slices.Equal(inp, keyByteMap["^C"]):
            a, b, valid, err := getRangeInp(oldState, texteditor.CopyUI)
            if err != nil {
                return err
            } else if valid {
        ...
        case slices.Equal(inp, keyByteMap["^Q"]):
            editLoop = false
        default:
            //checking if the key is a symbol or letter & not an unwanted ctrl/esc sequence
...
func runTextEditor() error {
    //arg validation
    path, ext, noHues, err := validateOsArgs()
    if err != nil {
    ...
    //ensuring the existence of a settings file if the args are valid & path != helpFlag
